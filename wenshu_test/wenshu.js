...
//这个cookie的值，直接从响应头中获取，传递给getKey.
...
//从Set-Cookie获取vjkl5的值
/*
//var cookie = getCookie('vjkl5');
function getCookie(name) {
    var arr, reg = new RegExp("(^| )" + name + "=([^;]*)(;|$)");
    document: 是浏览器在渲染页面成功以后，生成的一个对象，这个对象包含了请求成功后的所有信息，整个页面的标签结构，响应的状态，请求的状态。只在浏览器中存在。所以docmuent的功能，很想Python中的response对象。
...
//funIndex是一个数据的索引。% 在js中是取随机值。
var funIndex = strToLong(cookie) % arrFun.length;
//从数组arrFun中，利用索引随机获取一个加密函数。
var fun = arrFun[funIndex];
//将vjkl5这个cookie，传给fun这个加密函数，进行加密，加密后的结果就是vl5x的值。
